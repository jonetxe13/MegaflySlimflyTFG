...
long init_topo_slimfly(long np, long *par) {
    ...
    for (int i = 2; i<param_q; i++) {
        ...
        for (int j = 0; j<param_q-1; j++) {
            uint64_t prueba_gen = pow(param_gen, j);
    ...
    if(param_q%4==1){
        ...
        for (int i = 0; i<param_q-1; i++) {
            if(i%2==0) param_x[i/2]=mod((uint64_t)(pow(param_gen, i)),param_q);
            if(i%2!=0) param_x2[i/2]=mod((uint64_t)(pow(param_gen, i)),param_q);
        ...
        for (int i = 0; i<(2*param_l)-1; i+=2) {
            param_x[i/2]=mod(((uint64_t)pow(param_gen, i)),param_q);
            param_x[param_l+i/2]=mod(((uint64_t)pow(param_gen, 2*param_l-1+i)),param_q);
            // printf("param_x en %d: %d**%d \n", i/2, param_gen, i);
            // printf("param_x en %d: %d**%d\n", i/2, param_gen, 2*param_l-1+i);

            param_x2[i/2]=mod(((uint64_t)pow(param_gen, i+1)),param_q);
            param_x2[param_l+i/2]=mod(((uint64_t)pow(param_gen, 2*param_l+i)),param_q);
